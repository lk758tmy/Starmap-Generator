...
bool _dfs(int depth){//dfs search
//height equals to star's id, see above 
	...
	int x,y,i,height;
	height=(depth%2==0)?(63-depth/2):((depth-1)/2);
	for(x=1;x<33;x++){
		for(y=1;y<33;y++){
			if(starmap[x][y]!=0) continue;
			if(!__check(height,x,y)) continue;
			posX[depth].push_back(x);
			posY[depth].push_back(y);
		}
	}//Add all possible postion in a vector
	while(!posX[depth].empty()){
		i=rand()%posX[depth].size();
		x=posX[depth][i];
		y=posY[depth][i];
		posX[depth].erase(posX[depth].begin()+i);
		posY[depth].erase(posY[depth].begin()+i);
		__change_board(height,x,y,1);
		starX[height]=x; starY[height]=y;
		if(_dfs(depth+1)) return true;
		__change_board(height,x,y,-1);
	}//Randomly choose a position to continue
	return false;
}
int main(){
	_init();
