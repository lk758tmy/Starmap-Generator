...
int _rand_move(int height,int vector_max){
	int new_height,id_1,id_2=rand()%63;
	if(height<62) id_1=relocate[rand()%vector_max];
	else id_1=rand()%64;
	if(id_2>=id_1) id_2++;
	...
	if(A[sx_1]==0||B[sy_1]==0||C[sx_1+sy_1-16]==0)
		goto FAIL;
	if(A[sx_2]==0||B[sy_2]==0||C[sx_2+sy_2-16]==0)
		goto FAIL;	
	new_height=_check();
...
bool try_once(){
	int height,new_height,cnt,M_cnt,vm;
	do{
		_init();
		height=_rand_gene();
	}while(height<49);
	do{
		if(height<=61) M_cnt=12000001;
		else M_cnt=48000001;
		vm=_find_relocate();
		for(cnt=1;cnt<M_cnt;cnt++){
			if(cnt%3000000==0) printf("-");
			new_height=_rand_move(height,vm);
