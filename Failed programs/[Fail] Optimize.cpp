...
int starX[64],starY[64],starId[64],stars[64];
/*starmap[][] is the board (-1=empty, 0~63=star,
  ...
  starId is used for speed-up. See function _track()
  for what Id is. stars[]: Also see _track() (*) */
...
//Check how many different kinds of stars appear.
	...
	for(int i=0;i<64;i++){
		if(starId[i]==-1)
			starId[i]=_track(starX[i],starY[i]);
		if(stars[starId[i]]==0) cnt++;
		stars[starId[i]]++;
		//Record it for _find_relocate() to use.
	}
	return cnt;
}
void _add_star(int x,int y,int cnt){
	starX[cnt]=x; starY[cnt]=y; starmap[x][y]=cnt;
	A[x]++; B[y]++; C[x+y-16]++; 
	return ;
}
int _rand_setup(){
...
void _find_relocate(){//Record all repeat stars
	...
	for(int i=0;i<64;i++){
		stars[starId[i]]--;
		if(stars[starId[i]]>0) relocate.push_back(i);
	}
	return ;
}
void _change_board(int id,int ex,int ey){
	//Move the star at (starX[id],starY[id]) to (ex,ey)
	starId[id]=-1;//Only re-track "-1" stars in _track().
	int tx,ty;
	for(int i=0;i<6;i++){
		tx=starX[id]; ty=starY[id];
		while(1){
			tx+=dX[i]; ty+=dY[i];
			if(starmap[tx][ty]==-2) break;
			if(starmap[tx][ty]!=-1){
				starId[starmap[tx][ty]]=-1; break;
	}}}
	A[starX[id]]--; B[starY[id]]--; A[ex]++; B[ey]++;
	C[starX[id]+starY[id]-16]--; C[ex+ey-16]++;	
	starmap[starX[id]][starY[id]]=-1;
	starmap[ex][ey]=id; starX[id]=ex; starY[id]=ey;
	return ;
}
bool _check_necessary(int a,int b){
	int x=starX[a],y=starY[a];
...
/*Randomly relocate 2 stars. Doesn't work if only
  relocate 1 star, and costs more time if >=3.*/
	...
	id[1]=rand()%63;
	if(id[1]==id[0]) id[1]++;
	if(!_check_necessary(id[0],id[1])) return -1;
/*Do not waste time for doing the change if it doesn't
meet the necessary condition (see _rand_setup()).*/
	for(int i=0;i<2;i++){
		sx[i]=starX[id[i]]; sy[i]=starY[id[i]];
/*Record the original position, be able to redo if fails.*/
		do{
			ex[i]=rand()%32+1; ey[i]=rand()%32+1;
		}while(starmap[ex[i]][ey[i]]!=-1);
...
bool try_once(){
	...
	do{ //Part II: Random Hill-Climbing
		_find_relocate(); printf("%d ",height);
		for(cnt1=0;cnt1<12000000;cnt1++){
		...
		if(cnt1==12000000) return false;
	}while(cnt2<600); //Need a limit for safety.
	return false;
}
void _output(){
	for(int i=0;i<64;i++)
		printf("%c%c",starX[i]+48,starY[i]+48);
	return ;
}
int main() {
	...
	printf("Use the codes below to generate an image.\n");
	_output();
	getchar(); getchar();
	return 0;
}